"""
Wrapper module for parking sign OCR.
Contains OCR functions copied from parking_sign_ocr.py to avoid model loading at import time.
"""
import json
import re
...
OUTPUT ONLY THE JSON ARRAY (no other text):"""
    ...
    # When using device_map="auto", get device from first parameter
    try:
    ...
    # Generate output
    with torch.no_grad():
        # Remove temperature, top_p, top_k as they're not valid for this model
...
def extract_text_from_signs(image_path: Path, detections: List[Dict], model, processor) -> List[Dict[str, Any]]:
    """
    Extract text and arrow direction from detected parking signs.
